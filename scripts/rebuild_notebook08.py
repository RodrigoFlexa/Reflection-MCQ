"""Rebuild notebook 08 as the versioned multi-similarity experiment.

Keeping the notebook source in ordinary strings makes substantial revisions
reviewable in git while the generated .ipynb remains directly executable.
"""
...
TARGET = ROOT / "notebooks" / "08_similarity_reflection_threshold.ipynb"
...
4. duas reflexões answer-aware (`simple` e `complex`), com regra transferível
   no início e formato curto para modelos abaixo de 8B;
...
10. resultados separados por modelo, dataset e profundidade, sempre recuperando
    de **todas** as reflexões de origem (`pool=all`), acertadas ou erradas.
